...
####################################################################################
def Lbr_Melange2(x_BIB, y_BIB, z_BIB, t_BIB, R_BIB,x_top, y_top, z_top, t_top, R_top, TrueBIB, Truetop):
	x_mix= x_BIB + x_top
	y_mix= y_BIB + y_top
	z_mix= z_BIB + z_top
	t_mix= t_BIB + t_top
...
def Lbr_Clustering(parametre, R_mix,index1,VarT,VarZ,BIB):
    ...
    for i in tqdm(range(len(R_mix))):
        ...
        db = DBSCAN(eps=parametre, min_samples=2).fit(R1) #esp= distance entre chaque hit, min_samples -> nombre minimum de hit pour faire un cluster 
...
def Lbr_CleanCluster(Cluster1,Index_cluster1,Index_layer1,t_index1,z_index1,BIB_t):
    ...
    for f in range(len(Cluster1)): #on boucle sur les 10 event
        Cluster, Index_cluster, Index_layer, t_index, z_index, BIB_true = [], [], [], [], [], [] 
        Cluster2, Index_cluster2, Index_layer2, t_index2, z_index2, BIB_true2 =[], [], [], [], [], [] 
...
'''Création graph'''
####################################################################################
#exemple root pour un graphe dans un canevas avec un tracer dans le graphe: np.array(ctau_new)=axe des abscisses, np.array(eff1_and_tracker) axes des ordonnées.
...
def Lbr_AnalyseTrace2(Coef, Index_layer, BIB_true):
	...
	NombreTotalTraceTrue=0
	#HGTD gauche -> HGTD_G
	HGTD_G_NombreTotalTrace=0
	HGTD_G_NombreTotalTraceTrue=0
	HGTD_G_NombreBIB=0
	HGTD_G_NombreBIBTrue=0
	HGTD_G_NombreTop=0
	HGTD_G_NombreTopTrue=0
	
	#HGTD Droit -> HGTD_D
	HGTD_D_NombreTotalTrace=0
	HGTD_D_NombreTotalTraceTrue=0
	HGTD_D_NombreBIB=0
	HGTD_D_NombreBIBTrue=0
	HGTD_D_NombreTop=0
	HGTD_D_NombreTopTrue=0
	...
	for i in range(len(Coef)):
		if len(Coef[i]) > 0:
			for j in range(len(Coef[i])):
				#para=StandardScaler().inverse_transform(parametre)

				NombreTotalTrace += 1
				if Coef[i][j] > 0: #top
					HGTD_G_NombreTop += 1 
					if Tools.negatif(Index_layer[i][j]) and Tools.OneTop(BIB_true[i][j]):
						HGTD_G_NombreTopTrue += 1
						NombreTotalTraceTrue += 1
				if Coef[i][j] < 0: #Direction BIB (z decroissant)
					if Tools.negatif(Index_layer[i][j]): #HGTD Gauche					
						HGTD_G_NombreBIB += 1
						HGTD_G_NombreTotalTrace += 1
						if Tools.ZeroBIB(BIB_true[i][j]):
							HGTD_G_NombreBIBTrue += 1 
							NombreTotalTraceTrue += 1
							HGTD_G_NombreTotalTraceTrue += 1
					if Tools.positif(Index_layer[i][j]): #HGTD Droit
						HGTD_D_NombreTotalTrace += 1
						HGTD_D_NombreTop += 1
						if Tools.ZeroBIB(BIB_true[i][j]):
							NombreTotalTraceTrue += 1
							HGTD_D_NombreBIBTrue += 1
						if Tools.OneTop(BIB_true[i][j]):
							HGTD_D_NombreTotalTraceTrue += 1
							NombreTotalTraceTrue += 1
							HGTD_D_NombreTopTrue += 1
	pourcentage=((100*NombreTotalTraceTrue)/NombreTotalTrace)
	EFF_g = ((100*HGTD_G_NombreTotalTraceTrue)/HGTD_G_NombreTotalTrace)
	EFF_d = ((100*HGTD_D_NombreTotalTraceTrue)/HGTD_D_NombreTotalTrace)
	return HGTD_G_NombreTotalTrace, HGTD_G_NombreTotalTraceTrue, EFF_g, HGTD_D_NombreTotalTrace, HGTD_D_NombreTotalTraceTrue, EFF_d, 
